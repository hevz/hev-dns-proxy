...
int
SocketClient::sendMsg (int code, const char *msg, bool addErrno)
{
...
SocketClient::sendMsg (int code, const char *msg, bool addErrno, bool useCmdNum)
{
    ...
    if (addErrno) {
        if (useCmdNum) {
            ret = asprintf (&buf, "%d %d %s (%s)", code, getCmdNum (), msg,
                            strerror (errno));
        } else {
...
int
SocketClient::sendBinaryMsg (int code, const void *data, int len)
{
...
int
SocketClient::sendCode (int code)
{
    char buf[4];
...
char *
SocketClient::quoteArg (const char *arg)
{
    int len = strlen (arg);
...
int
SocketClient::sendDatav (struct iovec *iov, int iovcnt)
{
    pthread_mutex_lock (&mWriteMutex);
...
int
SocketClient::sendDataLockedv (struct iovec *iov, int iovcnt)
{
    if (mSocket < 0) {
        errno = EHOSTUNREACH;
        return -1;
