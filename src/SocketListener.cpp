...
void
SocketListener::init (const char *socketName, int socketFd, bool listen,
                      bool useCmdNum)
{
    mListen = listen;
...
int
SocketListener::startListener (int backlog)
{
    if (!mSocketName && mSock == -1) {
...
SocketListener::runListener ()
{
    ...
    while (1) {
        ...
        if ((rc = select (max + 1, &read_fds, NULL, NULL, NULL)) < 0) {
            if (errno == EINTR)
                continue;
            SLOGE ("select failed (%s) mListen=%d, max=%d", strerror (errno),
                   mListen, max);
            sleep (1);
            continue;
        } else if (!rc)
...
bool
SocketListener::release (SocketClient *c, bool wakeup)
{
    bool ret = false;
...
void
SocketListener::runOnEachSocket (SocketClientCommand *command)
{
    SocketClientCollection safeList;
