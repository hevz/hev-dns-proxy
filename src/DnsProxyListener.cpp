...
DnsProxyListener::GetAddrInfoHandler::GetAddrInfoHandler (
    ...
    unsigned netId)
    : mClient (c), mHost (host), mService (service), mHints (hints),
      mNetId (netId)
{
}
...
void *
DnsProxyListener::GetAddrInfoHandler::threadStart (void *obj)
{
...
static bool
sendBE32 (SocketClient *c, uint32_t data)
{
...
static bool
sendLenAndData (SocketClient *c, const int len, const void *data)
{
...
// Returns true on success
static bool
sendhostent (SocketClient *c, struct hostent *hp)
{
    bool success = true;
    int i;
    if (hp->h_name != NULL) {
    ...
    for (i = 0; hp->h_aliases[i] != NULL; i++) {
        success &=
            sendLenAndData (c, strlen (hp->h_aliases[i]) + 1, hp->h_aliases[i]);
    }
...
static bool
sendaddrinfo (SocketClient *c, struct addrinfo *ai)
{
    // struct addrinfo {
...
DnsProxyListener::GetAddrInfoHandler::run ()
{
    ...
    if (rv) {
        // getaddrinfo failed
        mClient->sendBinaryMsg (ResponseCode::DnsProxyOperationFailed, &rv,
                                sizeof (rv));
    } else {
        ...
        while (ai && success) {
            if (android_getapilevel () <= 22) { // Android 4.3 - 5.1
                success =
                    sendLenAndData (mClient, sizeof (struct addrinfo), ai) &&
...
DnsProxyListener::GetAddrInfoCmd::GetAddrInfoCmd (
    ...
    : NetdCommand ("getaddrinfo"), mDnsProxyListener (dnsProxyListener)
{
}

int
DnsProxyListener::GetAddrInfoCmd::runCommand (SocketClient *cli, int argc,
                                              char **argv)
{
    if (DBG) {
    ...
    if (ai_flags != -1 || ai_family != -1 || ai_socktype != -1 ||
        ai_protocol != -1) {
    ...
    if (DBG) {
        ALOGD ("GetAddrInfoHandler for %s / %s / {%u,%u}",
               name ? name : "[nullhost]", service ? service : "[nullservice]",
               netId, uid);
    }
...
DnsProxyListener::GetHostByNameCmd::GetHostByNameCmd (
    DnsProxyListener *dnsProxyListener)
    : NetdCommand ("gethostbyname"), mDnsProxyListener (dnsProxyListener)
{
}

int
DnsProxyListener::GetHostByNameCmd::runCommand (SocketClient *cli, int argc,
                                                char **argv)
{
    if (DBG) {
        for (int i = 0; i < argc; i++) {
    ...
    if (argc != 4) {
        char *msg = NULL;
        asprintf (&msg, "Invalid number of arguments to gethostbyname: %i",
                  argc);
        ALOGW ("%s", msg);
        cli->sendMsg (ResponseCode::CommandParameterError, msg, false);
        free (msg);
...
DnsProxyListener::GetHostByNameHandler::GetHostByNameHandler (
    ...
    : mClient (c), mName (name), mAf (af), mNetId (netId), mMark (mark)
{
}
...
void
DnsProxyListener::GetHostByNameHandler::start ()
{
    pthread_t thread;
    pthread_create (&thread, NULL,
...
void *
DnsProxyListener::GetHostByNameHandler::threadStart (void *obj)
{
    GetHostByNameHandler *handler =
        reinterpret_cast<GetHostByNameHandler *> (obj);
    handler->run ();
    delete handler;
    pthread_exit (NULL);
    return NULL;
...
DnsProxyListener::GetHostByNameHandler::run ()
{
    ...
    if (DBG) {
        ALOGD (
            "GetHostByNameHandler::run gethostbyname errno: %s hp->h_name = %s, name_len = %zu\n",
...
DnsProxyListener::GetHostByAddrCmd::GetHostByAddrCmd (
    const DnsProxyListener *dnsProxyListener)
    : NetdCommand ("gethostbyaddr"), mDnsProxyListener (dnsProxyListener)
{
}

int
DnsProxyListener::GetHostByAddrCmd::runCommand (SocketClient *cli, int argc,
                                                char **argv)
{
    if (DBG) {
        for (int i = 0; i < argc; i++) {
    ...
    if (argc != 5) {
        char *msg = NULL;
        asprintf (&msg, "Invalid number of arguments to gethostbyaddr: %i",
                  argc);
        ALOGW ("%s", msg);
        cli->sendMsg (ResponseCode::CommandParameterError, msg, false);
        free (msg);
    ...
    if (result <= 0) {
        char *msg = NULL;
        asprintf (&msg, "inet_pton(\"%s\") failed %s", addrStr,
                  strerror (errno));
        ALOGW ("%s", msg);
    ...
    DnsProxyListener::GetHostByAddrHandler *handler =
        new DnsProxyListener::GetHostByAddrHandler (
            cli, addr, addrLen, addrFamily, netId, MARK_UNSET);
    handler->start ();
...
DnsProxyListener::GetHostByAddrHandler::GetHostByAddrHandler (
    SocketClient *c, void *address, int addressLen, int addressFamily,
    unsigned netId, uint32_t mark)
    : mClient (c), mAddress (address), mAddressLen (addressLen),
      mAddressFamily (addressFamily), mNetId (netId), mMark (mark)
{
}
...
void
DnsProxyListener::GetHostByAddrHandler::start ()
{
    pthread_t thread;
    pthread_create (&thread, NULL,
...
void *
DnsProxyListener::GetHostByAddrHandler::threadStart (void *obj)
{
    GetHostByAddrHandler *handler =
        reinterpret_cast<GetHostByAddrHandler *> (obj);
    handler->run ();
    delete handler;
    pthread_exit (NULL);
    return NULL;
...
DnsProxyListener::GetHostByAddrHandler::run ()
{
    ...
    if (DBG) {
        ALOGD (
            "GetHostByAddrHandler::run gethostbyaddr errno: %s hp->h_name = %s, name_len = %zu\n",
            hp ? "success" : strerror (errno),
            (hp && hp->h_name) ? hp->h_name : "null",
            (hp && hp->h_name) ? strlen (hp->h_name) + 1 : 0);
    }
