...
static void
lookup_hostname (const char *hostname)
{
...
void
setup_proxy_uids (unsigned *uids, unsigned count, unsigned last_uid)
{
    if (!dns_proxy_policy) {
...
void
setup_dns (const char *dns1, const char *dns2)
{
    setup_dns_for_net (NETID_UNSET, dns1, dns2);
}

void
setup_dns_proxy (const char *dns1, const char *dns2)
{
...
void
setup_dns_for_net (unsigned netid, const char *dns1, const char *dns2)
{
    if (!dns1 || !strlen (dns1)) {
        dns1 = DNS_SERVER1;
    }
